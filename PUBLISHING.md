...
- Add the repository secrets `GPG_PRIVATE_KEY` and `PGP_PASSPHRASE` used by the release workflow.
...
- Run `make generate-check` and `make docs-validate`, then fix any generated documentation drift, frontmatter, or rendering issues before the first public tag.
...
- Commit the release version to [`provider/version/version.go`](provider/version/version.go) on `main`. The release workflow reads that constant, creates the matching SemVer tag if it does not already exist, and only then publishes the release artifacts.
- If the matching tag already exists in the repository, push-triggered workflow runs skip the release to avoid publishing the same version twice. Manual workflow runs on `main` publish assets for the existing tag by default.
...
* crazy-max/ghaction-import-gpg `@v7` MIT. Moving major tag, not immutable; see Note 3. Referenced from `.github/workflows/release.yml`. [GitHub](https://github.com/crazy-max/ghaction-import-gpg)
...
* goreleaser CLI `latest` MIT. Not version-pinned in this repo: `GNUmakefile` calls local `goreleaser`, and the release workflow step sets `version: latest`. [GitHub](https://github.com/goreleaser/goreleaser)
